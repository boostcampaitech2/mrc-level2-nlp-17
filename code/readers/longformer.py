...
class ModelLongForQuestionAnswering(AutoModelForQuestionAnswering):
    def __init__(self, config):
        super().__init__(config)
        self.roberta.encoder.layer = nn.ModuleList(
            [RobertaLongSelfAttention(config, layer_id=index) for index in range(config.num_hidden_layers)])
        # for i, layer in enumerate(self.roberta.encoder.layer):
        #     # replace the `modeling_bert.BertSelfAttention` object with `LongformerSelfAttention`
        #     layer.attention.self = LongSelfAttention(
        #         config,
        #         layer_id=i
        #     )
        # try:
